...
function displayValues(values){
    values.forEach((value) => console.log(value ** 3)); 	// Syntax: array.forEach((variable) => instructions)
};
...
function celsiusToFahrenheit(celcius_temp){
    let convertedArray = celcius_temp.map((temp) => (temp * 9 / 5.0) + 32);		// Syntax: array.map((variable) => instructions)
...
function areAllEven(numbers) {
    let isEven = numbers.every((num) => num%2 === 0);	// Syntax: array.every((variable) => condition)
    return isEven;
};
...
function hasDivisibleBy8(numbers){
    let hasDivisible = numbers.some((num) => num%8 === 0);	// Syntax: array.every((variable) => condition)
    return hasDivisible;
};
