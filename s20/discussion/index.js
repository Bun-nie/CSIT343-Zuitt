...
// returns a boolean value, if all elements meets the condition it returns true, otherwise false

let grades = [89, 85, 89, 90];
...
let fiveStarMovies = movies.filter(
	function(movie) {
		console.log(movie);
...
function filterQualifiedStudents(students) {
    return students
        .filter(student => student.score >= 80 && student.attendance)
        .map(student => `${student.name} has complete attendance and passed with a score of ${student.score}.`);
}


console.log(filterQualifiedStudents(students));

// Solution:

const passedStudents = students.filter(student => student.score >= 80 && student.attendance).map(student => student.name + " has complete attendance and passed with a score of " + student.score + ".");
