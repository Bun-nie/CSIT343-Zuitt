...
*/
function isPartIncluded(array, element){
    if (array.includes(element)){		// Syntax: array.includes(element)
    	return true;
    }
    return false;
};
...
function combineCart(cart1, cart2) {
    let combined_cart = cart1.concat(cart2); 	// Syntax: array1.concat(array2)
...
function arrayToString(array) {
	let to_string = array.toString();	// Syntax: array.toString();
	return to_string;
}
...
function joinArrays(array, separator) {
	let joined_string = array.join(separator);	// Syntax: array.join("[separator]"); default: "," (if no separator was specified)
